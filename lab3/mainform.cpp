...
bool Pearson(float fAlph, int iIntervals, float *fP, TMemo *mStat)
{
    ...
    if (fAlph <= 0.991 && fAlph > 0.976)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][5], ffFixed, 5, 5));
    if (fAlph <= 0.976 && fAlph > 0.951)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][4], ffFixed, 5, 5));
    if (fAlph <= 0.951 && fAlph > 0.051)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][3], ffFixed, 5, 5));
    if (fAlph <= 0.051 && fAlph > 0.026)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][2], ffFixed, 5, 5));
    if (fAlph <= 0.026 && fAlph > 0.011)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][1], ffFixed, 5, 5));
    if (fAlph <= 0.011 && fAlph > 0)
        mStat->Lines->Add("Табличное Хи квадрат - " + FloatToStrF(fPearson[s - 1][0], ffFixed, 5, 5));

    if (fAlph <= 0.991 && fHi < fPearson[s - 1][5])
        bValid = true;
    if (fAlph <= 0.976 && fHi < fPearson[s - 1][4])
        bValid = true;
    if (fAlph <= 0.951 && fHi < fPearson[s - 1][3])
        bValid = true;
    if (fAlph <= 0.051 && fHi < fPearson[s - 1][2])
        bValid = true;
    if (fAlph <= 0.026 && fHi < fPearson[s - 1][1])
        bValid = true;
    if (fAlph <= 0.011 && fHi < fPearson[s - 1][0])
        bValid = true;

    if (bValid == true)
        mStat->Lines->Add("Гипотеза о том, что распределение равномерно, верна.");
    else
        mStat->Lines->Add("Гипотеза о том, что распределение равномерно, НЕ верна.");
    return bValid;
}
//---------------------------------------------------------------------------
